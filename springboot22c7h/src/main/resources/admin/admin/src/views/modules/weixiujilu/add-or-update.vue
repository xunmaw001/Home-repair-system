	...
	<div class="addEdit-block" :style='{"padding":"30px 0 0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px","display":"inline-block"}' class="select" v-if="type!='info'"  label="维修记录" prop="weixiujilu">
					<el-select :disabled="ro.weixiujilu" v-model="ruleForm.weixiujilu" placeholder="请选择维修记录" >
						<el-option
							v-for="(item,index) in weixiujiluOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px","display":"inline-block"}' v-else class="input" label="维修记录" prop="weixiujilu">
					<el-input v-model="ruleForm.weixiujilu"
						placeholder="维修记录" readonly></el-input>
				</el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.weixiujiluOptions = "已维修,维修中".split(',')
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.weixiuxiangqing = this.ruleForm.weixiuxiangqing.replace(reg,'../../../springboot22c7h/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
